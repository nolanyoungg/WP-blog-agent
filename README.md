...
Generation is staged so a long article can resume section by section. First, a format-derived schema produces metadata and one topic-specific heading for every JSON-defined section. The plan prompt asks LM Studio to keep those sections distinct and use the configured conclusion as synthesis or a next step instead of another body topic. Each later call receives the complete format guidance, every rendered heading paired with its format purpose, and an explicit boundary that it owns only the current section. It returns the Markdown content for that section without repeating the article title or any section heading. Planning and section calls share factual-quality guidance that prohibits invented sources, guarantees, unsupported causal outcomes, arbitrary thresholds, and confusion between requirements and recommendations. The call also receives a rough share of the format's target to establish scale. `target_words` and the `avoid` list remain writing guidance, never deterministic pass/fail rules.

After all sections exist, the configured LM Studio model receives the complete assembled article in a separate senior-reviewer role. It returns either `pass` with an empty repair list or `revise` with explicit metadata or section repairs. Exact quoted text deterministically anchors each repair to the plan or correct section even if the model supplies the wrong index. A repair call must address every listed issue, then the complete article is reviewed again. The workflow repeats until nothing material remains. If the same quoted problem survives a targeted repair, the next repair replaces the affected plan or section completely; a third unresolved occurrence fails closed. An unresolved article is never saved as the final Markdown, rendered to PDF, or sent for approval.

After each valid plan, section, review list, or repair, an atomic checkpoint is saved beneath `CHECKPOINTS_DIR`, scoped to the tracker path and the complete JSON format definition. It preserves the current plan, sections, review round, pending repair list, issue-attempt history, and models used. If generation stops, the next claim resumes the unfinished generation or repair work instead of discarding it. A quality-passed Markdown draft removes its checkpoint. Structured retry feedback remains limited to repairing invalid JSON or required schema fields; editorial repairs come only from the explicit holistic review list.

The agent uses only `LMSTUDIO_PRIMARY_MODEL` (default `openai/gpt-oss-20b`) and makes up to three structured-output retries per writer, reviewer, or repair call. Task-specific instructions keep those roles distinct while using the same configured model. It will load that configured primary model if necessary, but it will not use or load a different model by default. Set `LMSTUDIO_ALLOW_FALLBACK_MODELS=true` only when you explicitly want other installed LLMs advertised by the same LM Studio server to be tried and, when necessary, loaded into memory. This never downloads a model; embeddings, Ollama, hosted OpenAI, Anthropic, and other providers remain excluded. Selected model, section progress, review verdicts, repair lists, checkpoint activity, retry errors, and completion are written to the run log and tracker. `LMSTUDIO_TIMEOUT_MS` defaults to five minutes, while `LMSTUDIO_MAX_TOKENS` defaults to 6,000 to bound a runaway response and produce an explicit completion error.
...
Before real model work, confirm server/model availability and stream LM Studio logs: `lms server status`, `lms log stream --source server --json`, and `lms log stream --source model --filter input,output --json --stats`. If no usable LM Studio LLM returns the required structured fields, or repeated material issues survive targeted repair and full replacement, the row becomes `error`, its valid checkpoint is retained, the final error is reported, and WordPress is untouched. The next `npm run once` retries an eligible generation-error row before a pending row. This repo’s policy prohibits fake model clients and canned model responses, so deterministic tests cover prompt, schema, parser, escalation, checkpoint, and file behavior; the complete writer-reviewer-repair flow must be verified against real LM Studio.
