...
Generation is staged so a long article can resume section by section. First, a format-derived schema produces metadata and one topic-specific heading for every JSON-defined section. Each later call receives the complete format guidance and returns the Markdown content for one section. It also receives a rough share of the format's target to establish scale. `target_words` remains guidance, never a pass/fail quota.
