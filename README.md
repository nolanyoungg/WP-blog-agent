...
Generation is staged so a long article can resume section by section. First, a format-derived schema produces metadata and one topic-specific heading for every JSON-defined section. The plan prompt asks LM Studio to keep those sections distinct and use the configured conclusion as synthesis or a next step instead of another body topic. Each later call receives the complete format guidance, every rendered heading paired with its format purpose, and an explicit boundary that it owns only the current section. It returns the Markdown content for that section without repeating the article title or any section heading. Planning and section calls share factual-quality guidance that prohibits invented sources, guarantees, unsupported causal outcomes, arbitrary thresholds, and confusion between requirements and recommendations. Because generation receives no authoritative source packet, it must not name external standards, versions, conformance levels, vendor guidelines, or numeric requirements from memory. The call also receives a rough share of the format's target to establish scale. `target_words` and the `avoid` list remain writing guidance, never deterministic pass/fail rules.

After all sections exist, the configured LM Studio model receives the complete assembled article in a separate senior-reviewer role. It returns either `pass` with an empty repair list or `revise` with explicit metadata or section repairs. Exact quoted text deterministically anchors each repair to the plan or correct section even if the model supplies the wrong index. Unique fuzzy anchoring tolerates Markdown punctuation differences while discarding stale findings that no longer identify current text. A repair call must address every listed issue, then the complete article is reviewed again. The workflow repeats until nothing material remains. A surviving issue first receives stronger targeted correction feedback, then triggers complete replacement of the affected plan or section if it persists, and finally fails closed if the replacement still does not resolve it. No-progress identity combines the section, category, and normalized underlying problem so superficial quote changes cannot reset escalation while a newly discovered issue receives its own repair lifecycle. An unresolved article is never saved as the final Markdown, rendered to PDF, or sent for approval.
