...
export const config = (overrides: Partial<Record<string, string>> = {}) => {
  ...
  return {
    ...
    messaging: { adapter: get('IMESSAGE_ADAPTER') ?? 'macos', recipient: get('IMESSAGE_RECIPIENT') ?? '', chatDb: (get('IMESSAGE_CHAT_DB') ?? '~/Library/Messages/chat.db').replace(/^~/, process.env.HOME ?? '') },
