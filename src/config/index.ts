...
export const config = (overrides: Partial<Record<string, string>> = {}) => {
  ...
  const bool = (name: string, fallback = false) => (get(name) ?? String(fallback)).toLowerCase() === 'true';
  const number = (name: string, fallback: number) => { const value = Number(get(name) ?? fallback); if (!Number.isFinite(value) || value < 0) throw new Error(`${name} must be a non-negative number`); return value; };
  ...
  return {
    ...
    editorialGuidancePath: path.resolve(get('EDITORIAL_GUIDANCE_PATH') ?? 'config/editorial-guidance.json'),
    checkpointsDir: path.resolve(get('CHECKPOINTS_DIR') ?? 'data/checkpoints'),
    draftsDir: path.resolve(get('DRAFTS_DIR') ?? 'data/drafts'),
    runsDir: path.resolve(get('RUNS_DIR') ?? 'data/runs'),
    ...
    messaging: {
      ...
      attachmentOutbox: path.resolve(expandHomeDirectory(get('IMESSAGE_ATTACHMENT_OUTBOX') ?? '~/Pictures/WP Blog Agent Outbox')),
      deliveryTimeoutMs: number('IMESSAGE_DELIVERY_TIMEOUT_MS', 60_000), deliveryPollMs: number('IMESSAGE_DELIVERY_POLL_MS', 250),
      relayUrl: (get('IMESSAGE_RELAY_URL') ?? '').replace(/\/$/, ''), relayToken: get('IMESSAGE_RELAY_TOKEN') ?? '', relayTimeoutMs: number('IMESSAGE_RELAY_TIMEOUT_MS', 90_000),
