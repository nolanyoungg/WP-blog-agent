...
export class BlogWorkflow {
  ...
  async processNext() {
    ...
    try {
      ...
      await this.log.write('workflow.editorial_guidance_selected', { blog_id: row.blog_id, rule_ids: guidance.ruleIds, source_ids: guidance.sourceIds });
      let checkpoint;
      ...
      const reviewArticle = async () => {
        ...
        return generatedReview.value;
      };
      let qualityReview = await reviewArticle();
      await this.log.write('workflow.quality_review_completed', { blog_id: row.blog_id, verdict: qualityReview.verdict, issue_count: qualityReview.issues.length });
      if (qualityReview.verdict === 'revise') {
        ...
        for (const [sectionNumber, issues] of issuesBySection) {
          ...
          await this.log.write('workflow.quality_repair_started', { blog_id: row.blog_id, section_index: sectionNumber, issue_count: issues.length });
...
Replace the prior section completely. Do not preserve a claim that the review identified as incorrect or unsupported.`,
          ...
          await this.log.write('workflow.quality_repair_succeeded', { blog_id: row.blog_id, section_index: sectionNumber, model: repaired.model });
        }
        qualityReview = await reviewArticle();
        await this.log.write('workflow.quality_review_completed', { blog_id: row.blog_id, verdict: qualityReview.verdict, issue_count: qualityReview.issues.length, after_repair: true });
        if (qualityReview.verdict !== 'pass') {
          const summary = qualityReview.issues.map(issue => `section ${issue.section_index}: ${issue.problem}`).join('; ');
          throw new Error(`Article quality review still requires revision after one repair pass: ${summary}`);
        }
      }
